...
func TestDecodeMinimalConfig(t *testing.T) {
	homeDir := t.TempDir()
	resolved, err := Decode([]byte(`{"profiles":{"personal":{"proxy":"socks5://127.0.0.1:1080"}}}`), homeDir)
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}

	profile, ok := resolved.Profiles["personal"]
	if !ok {
		t.Fatal("expected personal profile")
	}
	if profile.Proxy != "socks5://127.0.0.1:1080" {
		t.Fatalf("Proxy = %q", profile.Proxy)
	}
}

func TestDecodeTwoProfiles(t *testing.T) {
	resolved, err := Decode([]byte(`{"profiles":{"personal":{},"work":{}}}`), t.TempDir())
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}
...
func TestDefaultRuntimeRoot(t *testing.T) {
	homeDir := t.TempDir()
	resolved, err := Decode([]byte(`{"profiles":{"personal":{}}}`), homeDir)
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}
...
func TestCustomRuntimeRootAndTildeExpansion(t *testing.T) {
	homeDir := t.TempDir()
	resolved, err := Decode([]byte(`{"runtime_root":"~/custom-runtime","profiles":{"personal":{}}}`), homeDir)
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}
...
func TestRelativeStablePathsAreRejected(t *testing.T) {
	tests := []struct {
		...
		{
			name: "runtime root",
			data: `{"runtime_root":"relative-runtime","profiles":{"personal":{}}}`,
		},
		{
			name: "chatgpt app",
			data: `{"chatgpt_app":"relative/ChatGPT.app","profiles":{"personal":{}}}`,
		},
	}
...
func TestProfilePathsAreDerived(t *testing.T) {
	homeDir := t.TempDir()
	resolved, err := Decode([]byte(`{"runtime_root":"/tmp/runtime-root","profiles":{"personal":{}}}`), homeDir)
	if err != nil {
		t.Fatalf("Decode() error = %v", err)
	}

	profile := resolved.Profiles["personal"]
	if want := "/tmp/runtime-root/profiles/personal/codex-home"; profile.CodexHome != want {
		t.Fatalf("CodexHome = %q, want %q", profile.CodexHome, want)
	}
	if want := "/tmp/runtime-root/profiles/personal/chatgpt-user-data"; profile.UserDataDir != want {
...
func TestInvalidProxies(t *testing.T) {
	for _, proxy := range []string{
		...
		t.Run(proxy, func(t *testing.T) {
			...
			data := []byte(`{"profiles":{"personal":{"proxy":` + string(encodedProxy) + `}}}`)
...
func TestProxyPortsAreValidated(t *testing.T) {
	for _, proxy := range []string{
		...
		t.Run(proxy, func(t *testing.T) {
			data := []byte(`{"profiles":{"personal":{"proxy":` + mustMarshalString(proxy) + `}}}`)
			if _, err := Decode(data, t.TempDir()); err == nil {
...
func TestAllowedProxySchemes(t *testing.T) {
	for _, scheme := range []string{"socks5", "socks5h", "http", "https"} {
		t.Run(scheme, func(t *testing.T) {
			data := []byte(`{"profiles":{"personal":{"proxy":"` + scheme + `://127.0.0.1:1080"}}}`)
			if _, err := Decode(data, t.TempDir()); err != nil {
				t.Fatalf("Decode() error = %v", err)
			}
		})
...
func TestProxyCredentialsAreRejected(t *testing.T) {
	data := []byte(`{"profiles":{"personal":{"proxy":"socks5://user:password@127.0.0.1:1080"}}}`)
...
func TestUnknownFieldsAreRejected(t *testing.T) {
	for _, field := range []string{
		`{"profiles":{"personal":{}},"provider":"openai"}`,
		`{"profiles":{"personal":{"env":{}}}}`,
		`{"profiles":{"personal":{"codex_home":"/tmp/codex"}}}`,
		`{"profiles":{"personal":{"user_data_dir":"/tmp/chatgpt"}}}`,
		`{"profiles":{"personal":{"account":"work"}}}`,
		`{"profiles":{"personal":{"auth_token":"secret"}}}`,
		`{"profiles":{"personal":{"session":"session"}}}`,
	} {
